...
rows, columns = -1,-1#os.popen('stty size', 'r').read().split()
...
def image_transform(img, points, square_length=64,
				square_space_pts2 = np.float32([[0, 0], [8, 0], [8, 8], [0, 8]])):
	...
	pts2 = square_length*square_space_pts2
	max1=max(pt[0] for pt in pts2)
	min1=min(pt[0] for pt in pts2)
	max2=max(pt[1] for pt in pts2)
	min2=min(pt[1] for pt in pts2)
	...
	W = cv2.warpPerspective(img, M, (max1-min1,max2-min2))
...
class ImageObject(object):
	...
	def __init__(self, img, cum_trans = np.array([[1.,0.,0.],[0.,1.,0.],[0.,0.,1.]])):
	...
	def crop(self, pts):
		...
		img_crop, mat = image_transform(self.images['orig'], pts_orig)
		self.__init__(img_crop,mat.dot(self.cum_trans))
